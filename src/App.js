 ...
 fetchFunction()
  ...
  render() {
    ...
    const investorTypes = 
      [ {type: 'HODLler',
        ...
        plan: [
          {symbol: "BTC", procent: 80}, 
          {symbol: "LINK", procent: 10},
          {symbol: "ETH", procent: 5},
          {symbol: "USDT", procent: 3},
          {symbol: "ATOM", procent: 2}
        ]},
        {type: 'Early Investor',
        ...
        plan: [
          {symbol: "ETH", procent: 60}, 
          {symbol: "LINK", procent: 20},
          {symbol: "USDT", procent: 10},
          {symbol: "XMR", procent: 7},
          {symbol: "UNI", procent: 3}
        ]},
        {type: 'Trader',
        ...
        plan: [
          {symbol: "ADA", procent: 40}, 
          {symbol: "XMR", procent: 20},
          {symbol: "LINK", procent: 20},
          {symbol: "BTC", procent: 10},
          {symbol: "UNI", procent: 10}
        ]}
      ]
    ...
    const portfolioFirstBuild = (iSum, plan, marketData) => {
      ...
      for (let i = 0; i < plan.length; i++){
           for (let j = 0; j < marketData.length; j++ ) {
             if (plan[i].symbol === marketData[j].symbol) {
              firstPortfolio.push({coin: plan[i].symbol, icon: marketData[j].img, name: marketData[j].name, amount: ((iSum * plan[i].procent / 100) / marketData[j].quotes.USD.price), procent: `${plan[i].procent}%`});
             }
           }
        }
      }
  ...
  return (
    ...
    <div className="App">
      ...
      <Route exact path="/type" render= {()=><Type allTypes={investorTypes} investorType={this.state.investorType} sum={this.state.invSum} typeChoice={(type) => getInvestorType(type)} marketData={this.state.data} buildPortfolio={portfolioFirstBuild} />}></Route>
    </div>
    </Router>
